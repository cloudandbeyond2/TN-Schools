...
export default function ManageCommissionersPage() {
  ...
  const [isCommissionerDisabled, setIsCommissionerDisabled] = useState(false);
  ...
  const fetchCommissioners = async () => {
    try {
      const [resUsers, resFeatures] = await Promise.all([
        fetch(`${API_URL}/api/users?role=COMMISSIONER`),
      ...
      if (featData.success && featData.data?.portalVisibility) {
        setIsCommissionerDisabled(featData.data.portalVisibility.commissioner === false);
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      {/* Warning banner if Commissioner Portal is disabled in settings */}
      {isCommissionerDisabled && (
        <div className="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-300 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-bold text-amber-200">Commissioner Portal is Currently Disabled</div>
              <div className="text-[11px] text-amber-300/80 mt-0.5">The Commissioner portal, Home page section, and navigation links are hidden from general access. You can manage commissioner profiles here or re-enable the portal anytime.</div>
            </div>
          </div>
          <a
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <div className="text-2xl font-bold text-slate-800 dark:text-white">{isCommissionerDisabled ? "Disabled" : "Active"}</div>
          <span className={`badge px-2 py-0.5 rounded text-[10px] font-bold mt-2 inline-block ${
            isCommissionerDisabled
              ? "bg-amber-500/10 text-amber-500 dark:text-amber-400"
              : "bg-emerald-500/10 text-emerald-600 dark:text-emerald-300"
          }`}>
            {isCommissionerDisabled ? "Hidden from Portals" : "Full State Scope"}
          </span>
        </div>
      </div>
