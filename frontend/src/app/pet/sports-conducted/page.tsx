      ...
      <div className="p-4 sm:p-6 w-full space-y-6 text-slate-800 dark:text-slate-100">
            ...
            <div className="flex flex-wrap items-center gap-2.5 shrink-0">
              {events.length > 0 && (
                <button
                  ...
                  className="px-4 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-300 border border-rose-500/30 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all shadow-md hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
                  title="Clear all events to start with a blank calendar"
                >
                  <Trash2 size={14} className="text-rose-500" /> Clear Events
                </button>
              )}
              <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
          {LEVELS.map((level) => (
            <div 
              key={level} 
              className="relative overflow-hidden bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5 shadow-sm space-y-3 transition-all hover:shadow-md hover:-translate-y-0.5"
            >
              <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${LEVEL_GRADIENTS[level]}`} />
              <div>
                <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">{level}</div>
                <div className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight mt-1">
                  {levelStats.get(level) || 0}
                </div>
              </div>
              <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-1 truncate">
                <Users size={12} className="text-blue-500 shrink-0" /> Total Athletes
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm flex flex-col lg:flex-row items-stretch lg:items-center gap-3 sm:gap-4">
          
          {/* Kind Segmented Tabs */}
          <div className="flex p-1 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shrink-0 overflow-x-auto no-scrollbar">
            {(["All", "Event", "Competition"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`px-3.5 sm:px-4 py-1.5 text-xs font-bold rounded-xl transition-all whitespace-nowrap ${
                  tab === t
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
          ...
          {/* Header */}
          <div className="px-4 sm:px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40 flex flex-col sm:flex-row sm:items-center justify-between gap-2 font-bold text-xs">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
            {paginatedEvents.map((ev) => (
              <div 
                key={ev.id} 
                className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-all group"
              >
                <div className="flex items-start gap-3 sm:gap-4 min-w-0">
                  ...
                  <div className="min-w-0 space-y-1.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-bold text-sm sm:text-base text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors leading-snug">
                        {ev.name}
                      </h3>
                    ...
                    <div className="flex items-center gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400 font-medium flex-wrap">
                      <span className="flex items-center gap-1.5"><Medal size={13} className="text-blue-500 shrink-0" /> {ev.sport}</span>
                      <span className="flex items-center gap-1.5"><MapPin size={13} className="text-violet-500 shrink-0" /> {ev.level} · {ev.venue}</span>
                      <span className="flex items-center gap-1.5"><Calendar size={13} className="text-emerald-500 shrink-0" /> {ev.date}</span>
                    </div>
                ...
                <div className="flex items-center justify-between sm:justify-end gap-4 shrink-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800">
                  <div className="text-left sm:text-right space-y-0.5">
                    <div className="text-base sm:text-lg font-black text-slate-900 dark:text-white flex items-center gap-1.5 justify-start sm:justify-end">
