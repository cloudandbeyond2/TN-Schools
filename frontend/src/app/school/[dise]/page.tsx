        ...
        <div className="max-w-6xl mx-auto px-4 py-14 grid sm:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className="sm:col-span-2 lg:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-sm font-black text-white leading-tight">{school.name}</span>
            </div>
            <p className="text-xs text-slate-400 leading-relaxed">{portal?.tagline || "Excellence in Education, Rooted in Values"}</p>
          </div>

          <div>
            <h4 className="text-xs font-black text-white uppercase tracking-wider mb-4">Explore</h4>
            <ul className="space-y-2.5">
              {NAV.map((n) => (
                <li key={n.id}><a href={`#${n.id}`} className="text-xs text-slate-400 hover:text-white transition-colors flex items-center gap-1.5"><ChevronRight className="w-3 h-3" /> {n.label}</a></li>
              ))}
            </ul>
          </div>

          <div>
            <h4 className="text-xs font-black text-white uppercase tracking-wider mb-4">Contact</h4>
