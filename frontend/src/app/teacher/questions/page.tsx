...
export default function QuestionGeneratorPage() {
  ...
  const handleDownloadPDF = (folderName: string, folderQuestions: Question[]) => {
    try {
      ...
      // Title
      doc.setFontSize(16);
      const titleLines = doc.splitTextToSize(`Question Bank: ${folderName}`, 180);
      doc.text(titleLines, 14, 20);

      // Subtitle
      const headerOffset = 20 + (titleLines.length * 7);
      doc.setFontSize(10);
      doc.text(`Generated by TN Schools AI Ecosystem`, 14, headerOffset);

      let yPos = headerOffset + 12;
      ...
      folderQuestions.forEach((q, idx) => {
        ...
        if (q.type === 'mcq' && q.options) {
          q.options.forEach((opt, oIdx) => {
            ...
            // Clean option prefix if opt already starts with 'A)' or 'a)'
            const cleanOpt = opt.replace(/^[A-Za-z]\)\s*/, '');
            doc.text(`   ${String.fromCharCode(65 + oIdx)}) ${cleanOpt}`, 14, yPos);
            yPos += 6;
          });
          yPos += 4;
        } else {
          // Non-MCQ: Add blank dotted answer lines for students to write on printed paper
