...
import { Bot, Sparkles, RefreshCw, Clock, Flame, CheckCircle2, Circle, Target } from "lucide-react";
      ...
      <div className="glass rounded-2xl p-6 mb-6 border border-red-500/30 bg-gradient-to-br from-red-900/10 to-transparent fade-in">
        <div className="flex flex-col lg:flex-row lg:items-center gap-5 justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                Smart Revision Engine <Sparkles className="w-4 h-4 text-amber-400" />
              </h2>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              {generating ? (
                <><RefreshCw className="w-4 h-4 animate-spin" /> Analysing your data…</>
              ) : plan ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <div className="glass rounded-2xl p-6 border border-slate-700/50 fade-in-2 flex flex-col items-center justify-center text-center">
              ...
              <div className="mt-3 flex items-center gap-1.5 text-xs text-slate-400">
                <Clock className="w-3.5 h-3.5" /> {plan.dailyMinutes} min committed daily
              </div>
            </div>

            <div className="lg:col-span-2 glass rounded-2xl p-6 border border-slate-700/50 fade-in-2">
              <h3 className="text-base font-bold text-white mb-4 flex items-center gap-2">
                <Target className="w-4 h-4 text-red-400" /> AI Focus Areas
              </h3>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {complete ? (
                      <span className="text-[10px] font-black text-emerald-400 flex items-center gap-1">
                        <Flame className="w-3.5 h-3.5" /> DONE
                      </span>
                    ) : (
                    ...
                    {day.tasks.map((task: any, tIdx: number) => (
                      ...
                      >
                        {task.done ? (
                          ...
                          <Circle className="w-4 h-4 text-slate-600 group-hover:text-red-400 mt-0.5 shrink-0 transition-colors" />
                        )}
