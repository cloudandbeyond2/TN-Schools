    ...
    if (ch.generatedQuestions && ch.generatedQuestions.length > 0) {
      const mappedQuestions = ch.generatedQuestions.map((q: any) => {
        ...
        return {
          ...
          o: cleanOptions,
          a: ansIndex,
          exp: `This is a model solution generated by Smart Assistant for topic: "${ch.chapter}".`,
        };
      });
  ...
  const handleGenerateGeminiQuestions = async () => {
    if (!aiTopic.trim()) {
      return Swal.fire({
        ...
        text: "Please enter a topic title for Smart Assistant to generate practice questions.",
      });
    }
    ...
    try {
      ...
      if (data.success && data.data && data.data.length > 0) {
        const mappedQuestions = data.data.map((q: any) => {
          ...
          return {
            ...
            o: q.options || ["A", "B", "C", "D"],
            a: ansIndex,
            exp: `This is a model solution generated by Smart Assistant for topic: "${aiTopic.trim()}".`,
          };
        });
        ...
        Swal.fire({
          ...
          text: `Smart Assistant has generated 5 customized mock questions on "${aiTopic.trim()}". Starting practice...`,
          timer: 2000,
      ...
      Swal.fire({
        ...
        text: "Failed to connect to the Smart Assistant API.",
      });
    } finally {
      ...
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="flex gap-1 bg-slate-100 dark:bg-slate-900 rounded-xl p-1 w-fit">
          {([
            ...
            { key: "gemini", label: "AI Smart Assistant Generator", icon: "fi-rr-robot" },
        ...
        {activeTab === "gemini" && (
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-5 animate-in fade-in duration-200 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800 dark:text-white leading-tight">Smart Assistant Question Generator</h3>
