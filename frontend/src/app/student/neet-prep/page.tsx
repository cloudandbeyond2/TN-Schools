...
export default function StudentNEETPrepPage() {
  ...
  const startPracticeSession = async (ch: Chapter) => {
    Swal.fire({
      title: "Generating Practice...",
      text: `Gemini AI is crafting custom questions for ${ch.chapter}...`,
    ...
    try {
      const res = await fetch(`${API}/api/ai/generate-questions`, {
        ...
        body: JSON.stringify({
          ...
          mcqCount: ch.totalQuestions > 0 ? ch.totalQuestions : 10,
          shortCount: 0,
          longCount: 0,
        }),
      });
      ...
      console.error(e);
      // Fallback to hardcoded if AI fails or returns empty
      Swal.fire({
        icon: "warning",
        title: "AI Generation Failed",
        text: "Falling back to standard mock questions.",
        timer: 2000,
        showConfirmButton: false,
      });
      const fallbackQs = chapterQuestions[ch.subject] || chapterQuestions["Biology"];
      setPracticeQuestions(fallbackQs);
      setCurrentQIndex(0);
      setSelectedAns(null);
      setShowExplanation(false);
      setCorrectCount(0);
