...
export default function CommissionerInfrastructurePage() {
  ...
  const commUserId: string = (session?.user as any)?.id || "";
  ...
  const fetchReports = useCallback(async () => {
    if (!commUserId) return;
    setLoading(true); setError(null);
    try {
      const res = await fetch(`${API_BASE}/api/hierarchy/commissioner/${commUserId}/resource-reports`);
  ...
  const handleStatusUpdate = async (report: ResourceReport, newStatus: ReportStatus) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/hierarchy/commissioner/${commUserId}/resource-reports/${report.id}`, {
        method: "PATCH",
  ...
  const allDistricts = [...new Set(reports.map(r => r.schoolDistrict).filter(Boolean))];
  ...
  const districtEntries = summary?.byDistrict ? Object.entries(summary.byDistrict).sort((a, b) => b[1] - a[1]) : [];
    ...
    <PortalLayout>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6 fade-in">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">State-wide resource reports sent by headmasters directly to Commissioner</p>
        </div>
        <button onClick={fetchReports} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold rounded-xl transition-all shadow-md">🔄 Refresh</button>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6 fade-in">
          <div className="glass p-5 rounded-2xl border border-slate-800 flex flex-col gap-2">
            ...
            <div className="flex items-baseline gap-2"><span className="text-2xl font-black text-white">{summary.totalReports}</span><span className="text-[10px] text-cyan-400 font-bold">State-wide</span></div>
            <div className="text-[10px] text-slate-500">From <span className="text-cyan-300 font-semibold">{summary.schoolsReporting}</span> schools · <span className="text-cyan-300 font-semibold">{districtEntries.length}</span> districts</div>
          </div>
          <div className="glass p-5 rounded-2xl border border-slate-800 flex flex-col gap-2">
            ...
            {districtEntries.map(([dist, count]) => (
              <button key={dist} onClick={() => setDistrictFilter(districtFilter === dist ? "All" : dist)}
                className={`px-3 py-1.5 rounded-xl border text-[11px] font-bold transition-all ${
                  districtFilter === dist ? "bg-cyan-600 border-cyan-500 text-white" : "bg-slate-900 border-slate-700 text-slate-300 hover:border-cyan-500/50"
                }`}>
                {dist} <span className="opacity-70">({count})</span>
              </button>
            ))}
            {districtFilter !== "All" && (
              <button onClick={() => setDistrictFilter("All")} className="px-3 py-1.5 rounded-xl border border-slate-700 bg-slate-800 text-slate-400 text-[11px] font-bold">✕ Clear</button>
            )}
          </div>
        </div>
      )}
      ...
      <div className="glass rounded-2xl p-4 border border-slate-800 mb-5 fade-in">
        <div className="flex flex-wrap gap-3 items-center">
          <input type="text" placeholder="Search school or subject..." value={search} onChange={e => setSearch(e.target.value)}
            className="flex-1 min-w-[180px] bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500" />
      ...
      <div className="glass rounded-2xl border border-slate-800 overflow-hidden mb-6 fade-in">
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-white">📨 Reports from Schools — State Level</h3>
          <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Sent to Commissioner</span>
        </div>
        {loading ? (
          <div className="flex items-center justify-center py-16 text-slate-500 text-xs gap-3">
            <div className="w-5 h-5 rounded-full border-2 border-cyan-500/20 border-t-cyan-500 animate-spin" />Loading state-wide reports...
          </div>
        ) : error ? (
          ...
          <div className="py-14 text-center">
            ...
            <p className="text-slate-600 text-xs mt-1">{reports.length === 0 ? "Headmasters send infrastructure reports to Commissioner from their school portal" : "Try adjusting filters"}</p>
          </div>
        ) : (
                ...
                {filtered.map(r => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors cursor-pointer" onClick={() => setDetailReport(r)}>
                      <td className="px-5 py-3.5"><div className="font-bold text-white">{r.schoolName}</div><div className="text-[10px] text-slate-500">{r.schoolBlock}</div></td>
                      <td className="px-4 py-3.5"><span className="text-slate-300 text-[11px] font-semibold">{r.schoolDistrict}</span></td>
                      <td className="px-4 py-3.5 max-w-[200px]">
                        <div className="text-[10px] text-cyan-400 font-semibold mb-0.5">{TYPE_ICON[r.reportType]} {r.reportType}</div>
                        <div className="text-white text-[11px] line-clamp-1 font-medium">{r.subject}</div>
                      </td>
                      <td className="px-4 py-3.5"><span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-bold ${pm.chip}`}><span className={`w-1.5 h-1.5 rounded-full ${pm.dot}`} />{r.priority}</span></td>
                      <td className="px-4 py-3.5"><span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold ${sm.badge}`}><span className={`w-1.5 h-1.5 rounded-full ${sm.dot}`} />{r.status}</span></td>
                      ...
                      <td className="px-4 py-3.5 text-right" onClick={e => e.stopPropagation()}>
                        {sm.next ? (
                          <button disabled={isUpdating} onClick={() => handleStatusUpdate(r, sm.next!)}
                            className="px-3 py-1.5 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 text-white text-[10px] font-bold rounded-lg transition-all whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setDetailReport(null)}>
          <div className="w-full max-w-xl rounded-3xl p-6 space-y-4" style={{ background: "#090d16", border: "1px solid rgba(255,255,255,0.12)", boxShadow: "0 25px 60px rgba(0,0,0,0.95)" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between">
              <div>
                <div className="text-xs font-bold text-cyan-400 mb-1">{TYPE_ICON[detailReport.reportType]} {detailReport.reportType}</div>
                <h3 className="text-base font-bold text-white">{detailReport.subject}</h3>
                <p className="text-xs text-slate-400 mt-1">From: <span className="text-cyan-300 font-semibold">{detailReport.schoolName}</span> · <span className="text-slate-500">{detailReport.schoolDistrict}</span></p>
              </div>
              <button onClick={() => setDetailReport(null)} className="text-slate-400 hover:text-white text-xs">✕ Close</button>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-bold ${PRIORITY_META[detailReport.priority].chip}`}><span className={`w-1.5 h-1.5 rounded-full ${PRIORITY_META[detailReport.priority].dot}`} />{detailReport.priority} Priority</span>
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold ${STATUS_META[detailReport.status].badge}`}><span className={`w-1.5 h-1.5 rounded-full ${STATUS_META[detailReport.status].dot}`} />{detailReport.status}</span>
            ...
            {detailReport.description && <div className="bg-slate-900/60 rounded-xl p-4 border border-slate-800"><div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-2">Description</div><p className="text-xs text-slate-300 leading-relaxed">{detailReport.description}</p></div>}
              ...
              <div className="bg-slate-900/60 rounded-xl p-4 border border-slate-800">
                ...
                <div className="grid grid-cols-4 gap-2 text-xs">
                  {[{l:"Total",v:detailReport.snapshot?.totalResources},{l:"Functional",v:detailReport.snapshot?.functional,c:"text-emerald-400"},{l:"Needs Repair",v:detailReport.snapshot?.needsRepair,c:"text-amber-400"},{l:"Critical",v:detailReport.snapshot?.critical,c:"text-rose-400"}].map(item=>(
                    <div key={item.l} className="bg-slate-800/40 rounded-lg p-2 text-center"><div className="text-[9px] text-slate-500 mb-1">{item.l}</div><div className={`text-sm font-black ${item.c||"text-white"}`}>{item.v??"—"}</div></div>
                  ))}
                </div>
              </div>
            )}
            {STATUS_META[detailReport.status].next && (
              <button disabled={updatingId === detailReport.id} onClick={() => handleStatusUpdate(detailReport, STATUS_META[detailReport.status].next!)}
                className="w-full py-3 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 text-white text-sm font-bold rounded-xl transition-all">
