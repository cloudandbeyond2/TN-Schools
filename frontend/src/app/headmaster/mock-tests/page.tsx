...
export default function HeadmasterMockTestsPage() {
  ...
  const normalizeGrade = (g: string) => (g || "").toLowerCase().replace(/grade\s*|class\s*/g, "").trim();
  ...
  }, [filteredTests, currentPage, ITEMS_PER_PAGE]);
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {loading ? (
            ...
            <>
              ...
              {totalPages > 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                  <div className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">
                    {lang === "தமிழ்" 
                      ? `பக்கம் ${currentPage} / ${totalPages} (${filteredTests.length} தேர்வுகள்)` 
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-bold bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                    >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
                      <button
                        ...
                        className={`w-8 h-8 sm:w-9 sm:h-9 text-xs sm:text-sm font-bold rounded-xl transition-colors ${
                          currentPage === pageNum
