...
export default function HeadmasterClubMembersPage() {
  ...
  return (
    <PortalLayout 
      title={lang === "தமிழ்" ? "மன்ற உறுப்பினர்கள்" : "Club Members Directory"} 
      subtitle={lang === "தமிழ்" ? "பள்ளி மன்ற உறுப்பினர்களின் மேலோட்டம்" : "Comprehensive overview of all student club members"} 
    ...
    >
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm text-left">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <Link href="/headmaster/clubs" className="text-xs font-bold text-slate-500 hover:text-blue-500 transition-colors flex items-center gap-1">
            <i className="fi fi-rr-arrow-left"></i> Back to Clubs
          </Link>
        </div>
                ...
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all border-2 ${
                    activeTab === clubName 
                      ...
                      : 'bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-blue-400/50 hover:text-blue-600'
                  }`}
                >
                  {clubName}
                </button>
                      ...
                      currentMembers.map((member, idx) => (
                        <tr key={member.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                          ...
                          <td className="px-6 py-4 font-bold text-slate-800 dark:text-white flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 flex items-center justify-center text-xs font-bold shrink-0">
