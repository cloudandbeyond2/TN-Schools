...
export default function HeadmasterAttendancePage() {
  ...
  return (
    ...
    >
      {/* Search & Filter Panel */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 border border-slate-200 dark:border-slate-800 mb-6 space-y-4 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <SlidersHorizontal className="w-5 h-5 text-blue-600 dark:text-blue-400" />
            <h2 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white uppercase tracking-wider">Dashboard Filters</h2>
          </div>
          <button 
            onClick={handleResetFilters}
            className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg text-xs font-semibold transition-all"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="flex flex-col space-y-1.5 text-left">
            <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Class</label>
            <select
              value={selectedClass}
              onChange={(e) => setSelectedClass(e.target.value)}
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500 cursor-pointer transition-colors"
            >
          ...
          {/* Section Filter */}
          <div className="flex flex-col space-y-1.5 text-left">
            <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Section</label>
            <select
          ...
          {/* Date Picker */}
          <div className="flex flex-col space-y-1.5 text-left">
            <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Target Date</label>
          ...
          {/* Search Box */}
          <div className="flex flex-col space-y-1.5 text-left">
            <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">Search Student</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs font-semibold text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
              />
            </div>
          </div>
        </div>
        ...
        {[
          {
            ...
            bg: "bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-800",
          ...
          },
          {
            ...
            icon: <UserMinus className="w-5 h-5 text-rose-600 dark:text-rose-400" />,
            bg: "bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-800",
            iconBg: "bg-rose-50 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400",
          ...
          <div 
            key={i} 
            className={`rounded-2xl p-4 border flex items-center justify-between hover:scale-[1.02] hover:-translate-y-0.5 hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-300 shadow-sm ${card.bg}`}
          >
            <div className="flex flex-col text-left min-w-0">
              <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider truncate">{card.label}</span>
              <span className="text-lg sm:text-2xl font-black text-slate-900 dark:text-white mt-1.5">{card.value}</span>
      ...
      {/* Tabs Menu Panel */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 mb-6 overflow-hidden shadow-sm">
        <div className="flex border-b border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-900/40 p-2 overflow-x-auto gap-2">
          {[
            ...
            <>
              ...
              {activeTab === "daily" && (
                <div className="space-y-4 text-left">
                  {/* Class & Section Classwise Interactive Selector */}
                  <div className="space-y-3 bg-slate-50/80 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <span className="text-xs font-bold text-slate-800 dark:text-slate-300 uppercase tracking-wider flex items-center gap-2">
                        ...
                        Select Class & Section to View Student Roster
                      </span>
                      <span className="text-[11px] font-bold text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 px-2.5 py-1 rounded-lg w-fit">
                        {selectedClass === "All" ? "All Classes" : `Class ${selectedClass}`} {selectedSection === "All" ? "(All Sections)" : `· Section ${selectedSection}`}
                      </span>
                    </div>
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-xl border text-center transition-all cursor-pointer ${
                          selectedClass === "All"
                            ...
                            : "bg-white dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-blue-400 dark:hover:border-slate-700 shadow-sm"
                        }`}
                      >
                        <div className="text-xs font-bold">All Classes</div>
                        <div className={`text-[10px] mt-0.5 ${selectedClass === "All" ? "text-blue-100 font-semibold" : "text-slate-500 dark:text-slate-400"}`}>
                          {data?.dailyLogs?.length || 0} Students
                        </div>
                      </button>
                      {classesList.map((cls) => {
                        const classLogs = data?.dailyLogs?.filter(l => l.class === cls) || [];
                        ...
                        return (
                          <button
                            ...
                            className={`p-2.5 rounded-xl border text-center transition-all cursor-pointer ${
                              isSelected
                                ? "bg-blue-600 border-blue-500 text-white shadow-md font-bold"
                                : "bg-white dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-blue-400 dark:hover:border-slate-700 shadow-sm"
                            }`}
                          >
                            <div className="text-xs font-bold">Class {cls}</div>
                            <div className={`text-[10px] mt-0.5 ${isSelected ? "text-blue-100 font-semibold" : "text-slate-500 dark:text-slate-400"}`}>
                              {classLogs.length} Students
                            </div>
                          </button>
                        );
                      })}
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-200 dark:border-slate-800/60">
                      <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 mr-1">Section:</span>
                      <button
                        type="button"
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Daily Attendance Grid</h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Edit status or verify daily marks. Total matched: {filteredDailyLogs.length}</p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-500 dark:text-slate-400">Status Filter:</span>
                        <select
                          value={statusFilter}
                          onChange={(e) => setStatusFilter(e.target.value)}
                          className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-200 cursor-pointer focus:outline-none focus:border-blue-500"
                        >
                  ...
                  {filteredDailyLogs.length === 0 ? (
                    <div className="text-center py-16 text-slate-500 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
                      ...
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-300">No attendance records found matching filters.</p>
                      <p className="text-[10px] text-slate-500 dark:text-slate-500 mt-1">Try resetting filter selectors or picking a different date.</p>
                    </div>
                  ) : (
                    <>
                      {/* Desktop Table View */}
                        ...
                        <table className="data-table">
                          ...
                          <tbody>
                            {paginatedLogs.map((log) => (
                              <tr key={log.studentId} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                                <td className="font-mono text-[11px] font-bold text-slate-600 dark:text-slate-400">{log.rollNumber}</td>
                                <td className="font-bold text-slate-900 dark:text-white text-xs">{log.name}</td>
                                <td>
                                  ...
                                  {isUpdating === log.studentId ? (
                                    ...
                                    <div className="inline-flex gap-1.5">
                                      <button
                                        ...
                                        className="px-2 py-1 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-500 hover:text-white text-emerald-600 dark:text-emerald-400 rounded-lg text-[10px] font-bold transition-all border border-emerald-200 dark:border-emerald-500/20 disabled:opacity-30"
                                      >
                                        Present
                                      </button>
                                      <button
                                        ...
                                        className="px-2 py-1 bg-rose-50 dark:bg-rose-500/10 hover:bg-rose-500 hover:text-white text-rose-600 dark:text-rose-400 rounded-lg text-[10px] font-bold transition-all border border-rose-200 dark:border-rose-500/20 disabled:opacity-30"
                                      >
                                        Absent
                                      </button>
                                      <button
                                        ...
                                        className="px-2 py-1 bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-500 hover:text-white text-amber-600 dark:text-amber-400 rounded-lg text-[10px] font-bold transition-all border border-amber-200 dark:border-amber-500/20 disabled:opacity-30"
                                      >
                                        Late
                                      </button>
                                    </div>
                      ...
                      <div className="block md:hidden space-y-3">
                        {paginatedLogs.map((log) => (
                          <div 
                            key={log.studentId} 
                            className="p-4 bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl space-y-3 flex flex-col text-left shadow-sm"
                          >
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="font-bold text-slate-900 dark:text-white text-xs">{log.name}</h4>
                              ...
                              {isUpdating === log.studentId ? (
                                ...
                                <div className="flex gap-1">
                                  <button
                                    onClick={() => handleStatusChange(log.studentId, "PRESENT")}
                                    disabled={log.status === "PRESENT"}
                                    className="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-500 hover:text-white text-emerald-600 dark:text-emerald-400 rounded-md text-[9px] font-bold transition-all border border-emerald-200 dark:border-emerald-500/20 disabled:opacity-30"
                                  >
                                    Present
                                  </button>
                                  <button
                                    onClick={() => handleStatusChange(log.studentId, "ABSENT")}
                                    disabled={log.status === "ABSENT"}
                                    className="px-2 py-0.5 bg-rose-50 dark:bg-rose-500/10 hover:bg-rose-500 hover:text-white text-rose-600 dark:text-rose-400 rounded-md text-[9px] font-bold transition-all border border-rose-200 dark:border-rose-500/20 disabled:opacity-30"
                                  >
                                    Absent
                                  </button>
                                  <button
                                    onClick={() => handleStatusChange(log.studentId, "LATE")}
                                    disabled={log.status === "LATE"}
                                    className="px-2 py-0.5 bg-amber-50 dark:bg-amber-500/10 hover:bg-amber-500 hover:text-white text-amber-600 dark:text-amber-400 rounded-md text-[9px] font-bold transition-all border border-amber-200 dark:border-amber-500/20 disabled:opacity-30"
                                  >
                                    Late
                                  </button>
                                </div>
                      ...
                      {/* Pagination Controls */}
                      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-6 pt-4 border-t border-slate-200 dark:border-slate-800/40">
                        <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <select
                              ...
                              className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1 text-[11px] font-semibold text-slate-800 dark:text-slate-300 focus:outline-none focus:border-slate-400 dark:focus:border-slate-700"
                            >
                        ...
                        {totalPages > 0 && (
                          <div className="flex space-x-2">
                            <button
                              ...
                              className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 text-slate-700 dark:text-slate-300 text-xs rounded-xl transition-colors border border-slate-200 dark:border-slate-700 disabled:cursor-not-allowed font-semibold flex items-center gap-1"
                            >
                              <ChevronLeft className="w-3.5 h-3.5" /> Previous
                            </button>
                            <div className="px-3.5 py-1.5 bg-slate-100 dark:bg-slate-900/50 text-slate-700 dark:text-slate-400 text-xs rounded-xl border border-slate-200 dark:border-slate-800 flex items-center font-bold">
