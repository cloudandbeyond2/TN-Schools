...
export const CENTER_CONTENT: Record<string, TopicContent> = {
  ...
  "Concepts:Tables": {
    title: "Tables", emoji: "🔲",
    summary: "A table is a collection of related data held in a structured format within a database. It consists of columns and rows.",
    keyPoints: ["Columns represent fields or attributes", "Rows represent records or entries", "Each table should have a primary key to uniquely identify rows", "Tables are the fundamental building blocks of a relational database"],
    funFact: "The first relational database was created by E. F. Codd in 1970 at IBM!",
    quiz: { question: "What does a row in a table represent?", options: ["A column", "A record", "A field", "A data type"], answer: "A record" },
  },
  "Concepts:Keys": {
    title: "Keys", emoji: "🔑",
    summary: "Keys are used to establish and identify relationships between tables. The primary key uniquely identifies a row, while a foreign key links to another table.",
    keyPoints: ["Primary Key (PK): unique and non-null identifier for a row", "Foreign Key (FK): a column that refers to the PK of another table", "Composite Key: a key that consists of multiple columns", "Unique Key: ensures all values in a column are different"],
    funFact: "A foreign key doesn't have to be unique, but it must match a primary key in another table!",
    quiz: { question: "Which key is used to uniquely identify each row in a table?", options: ["Foreign Key", "Primary Key", "Composite Key", "Unique Key"], answer: "Primary Key" },
  },
  "Concepts:Relationships": {
    title: "Relationships", emoji: "🔗",
    summary: "Relationships define how data in one table is connected to data in another table. The most common types are one-to-one, one-to-many, and many-to-many.",
    keyPoints: ["One-to-One: A single record in Table A relates to a single record in Table B", "One-to-Many: A single record in Table A relates to multiple records in Table B", "Many-to-Many: Multiple records in Table A relate to multiple records in Table B", "Many-to-Many relationships usually require a junction (bridge) table"],
    funFact: "A teacher having many students is a classic example of a one-to-many relationship!",
    quiz: { question: "What type of relationship is established using a junction table?", options: ["One-to-One", "One-to-Many", "Many-to-Many", "None"], answer: "Many-to-Many" },
  },
  "Concepts:Normalization": {
    title: "Normalization", emoji: "🧹",
    summary: "Normalization is the process of organising data in a database to reduce redundancy and improve data integrity.",
    keyPoints: ["1NF: Eliminate repeating groups, ensure atomic values", "2NF: Ensure all non-key attributes are fully functional dependent on the primary key", "3NF: Eliminate transitive dependencies", "Helps in reducing data anomalies during insertions, updates, or deletions"],
    funFact: "Boyce-Codd Normal Form (BCNF) was introduced to handle cases where 3NF is not sufficient!",
    quiz: { question: "What is the main goal of database normalization?", options: ["Increase redundancy", "Reduce data redundancy", "Slow down queries", "Encrypt data"], answer: "Reduce data redundancy" },
  },
  "SQL:INSERT": {
    title: "SQL INSERT Statement", emoji: "➕",
    summary: "The INSERT INTO statement is used to add new rows of data into a table in the database.",
    keyPoints: ["Syntax: INSERT INTO table_name (column1, column2) VALUES (value1, value2);", "You must provide values for all columns unless they have default values or allow NULL", "You can insert multiple rows at once by separating value sets with commas", "Strings and dates should be enclosed in single quotes"],
    formula: "INSERT INTO students (name, age) VALUES ('Alice', 15);",
    funFact: "You can also use 'INSERT INTO ... SELECT ...' to copy data from one table to another!",
    quiz: { question: "Which SQL statement is used to add new records to a database?", options: ["UPDATE", "ADD", "INSERT INTO", "CREATE"], answer: "INSERT INTO" },
  },
  "SQL:GROUP BY": {
    title: "SQL GROUP BY Statement", emoji: "📊",
    summary: "The GROUP BY statement groups rows that have the same values in specified columns into summary rows, often used with aggregate functions.",
    keyPoints: ["Aggregate functions like COUNT(), MAX(), MIN(), SUM(), AVG() are often used with GROUP BY", "The HAVING clause is used to filter records after grouping (since WHERE cannot be used with aggregates)", "Groups data based on one or multiple columns", "Essential for generating reports and data analysis"],
    formula: "SELECT class, COUNT(*) FROM students GROUP BY class;",
    funFact: "Without GROUP BY, aggregate functions return a single value for the entire table!",
    quiz: { question: "Which clause is used to filter records after grouping them?", options: ["WHERE", "ORDER BY", "HAVING", "LIMIT"], answer: "HAVING" },
  },
  "SQL:Query Runner": {
    title: "Query Runner", emoji: "⌨️",
    summary: "A query runner allows you to execute SQL statements directly against a database and view the results in real-time.",
    keyPoints: ["Write raw SQL queries", "Execute SELECT, INSERT, UPDATE, and DELETE statements", "View table results and affected rows", "Understand error messages and syntax issues interactively"],
    funFact: "Many modern query runners use WebAssembly (like SQL.js) to run a full database entirely in your web browser!",
    quiz: { question: "What can you do with a query runner?", options: ["Only view tables", "Execute raw SQL statements", "Design UI forms", "Format CSS"], answer: "Execute raw SQL statements" },
  },
  "Practice:Sample Database": {
    title: "Sample Database", emoji: "🗄️",
    summary: "A sample database provides a pre-populated set of tables and data for you to practice writing SQL queries without fear of breaking a real system.",
    keyPoints: ["Includes tables like Students, Teachers, Classes, and Marks", "Pre-filled with realistic mock data", "Helps in understanding table structures and relationships", "Great starting point for learning complex JOINs and aggregations"],
    funFact: "The 'Northwind' database is one of the most famous sample databases, originally created by Microsoft in the 1990s!",
    quiz: { question: "Why is a sample database useful?", options: ["To store actual student passwords", "To practice SQL queries safely", "To increase website speed", "To learn CSS"], answer: "To practice SQL queries safely" },
  },
  "Practice:Challenges": {
    title: "SQL Challenges", emoji: "🏆",
    summary: "SQL challenges test your ability to write correct queries to solve specific problems or retrieve specific data sets.",
    keyPoints: ["Problems range from simple SELECT statements to complex multi-table JOINs", "Tests your understanding of aggregate functions and GROUP BY", "Provides immediate feedback on your query results", "Improves your problem-solving skills in database management"],
    funFact: "Competitive SQL programming exists, where developers race to write the most efficient queries!",
    quiz: { question: "What do SQL challenges primarily test?", options: ["Your typing speed", "Your ability to write correct SQL queries", "Your UI design skills", "Your knowledge of hardware"], answer: "Your ability to write correct SQL queries" },
  },
  ...
  "Foundations:Datasets": {
    title: "Datasets", emoji: "🗂️",
    summary: "A dataset is a structured collection of data used to train and test machine learning models. Without good data, AI models cannot learn effectively.",
    keyPoints: ["Training Set: The data used to teach the model", "Validation Set: Used to tune the model's settings (hyperparameters)", "Test Set: Unseen data used to evaluate final performance", "Garbage In, Garbage Out: Poor data quality leads to poor AI predictions"],
    funFact: "ImageNet is one of the most famous datasets, containing over 14 million annotated images!",
    quiz: { question: "Which portion of a dataset is used to evaluate the final performance of a model?", options: ["Training Set", "Validation Set", "Test Set", "Feature Set"], answer: "Test Set" },
  },
  "Foundations:Features": {
    title: "Features", emoji: "🔧",
    summary: "In machine learning, a feature is an individual measurable property or characteristic of a phenomenon being observed.",
    keyPoints: ["Features are the input variables provided to a model (like columns in a spreadsheet)", "Feature Engineering: The process of creating new features to improve model accuracy", "Labels (or Targets): The output the model is trying to predict", "Too many irrelevant features can confuse a model (Curse of Dimensionality)"],
    funFact: "Sometimes combining two simple features into one complex feature can drastically improve an AI's intelligence!",
    quiz: { question: "What do we call the output that a supervised learning model tries to predict?", options: ["Feature", "Label", "Dataset", "Weight"], answer: "Label" },
  },
  "Foundations:Ethics": {
    title: "AI Ethics", emoji: "⚖️",
    summary: "AI Ethics involves ensuring that artificial intelligence systems are developed and used in ways that are safe, fair, and beneficial to humanity.",
    keyPoints: ["Bias: AI can inherit prejudices present in the training data", "Transparency: It should be clear how an AI makes its decisions (Explainable AI)", "Privacy: AI must respect user data and confidentiality", "Safety: Ensuring autonomous systems (like self-driving cars) do not cause harm"],
    funFact: "In 2016, an AI chatbot was taken offline in less than 24 hours because it learned toxic behavior from users on social media!",
    quiz: { question: "When an AI model inherits prejudices from its training data, what is this called?", options: ["Transparency", "Fairness", "Bias", "Optimization"], answer: "Bias" },
  },
  "Learn:Classification": {
    title: "Classification", emoji: "🏷️",
    summary: "Classification is a supervised learning approach where the model predicts a discrete category or label for a given input.",
    keyPoints: ["Binary Classification: Predicting between two classes (e.g., Spam vs. Not Spam)", "Multi-class Classification: Predicting between more than two classes (e.g., Cat, Dog, or Bird)", "Common algorithms: Logistic Regression, Decision Trees, Support Vector Machines", "Evaluated using metrics like Accuracy, Precision, and Recall"],
    funFact: "Spam filters in your email were one of the earliest successful applications of text classification!",
    quiz: { question: "Predicting whether an email is 'Spam' or 'Not Spam' is an example of what?", options: ["Regression", "Classification", "Clustering", "Reinforcement Learning"], answer: "Classification" },
  },
  "Learn:Regression": {
    title: "Regression", emoji: "📈",
    summary: "Regression is a supervised learning approach used to predict a continuous numerical value rather than a category.",
    keyPoints: ["Used for predicting prices, temperatures, or test scores", "Linear Regression: Fits a straight line to the data", "Polynomial Regression: Fits a curve for more complex relationships", "Evaluated using metrics like Mean Squared Error (MSE)"],
    formula: "y = mx + c (Equation of a straight line)",
    funFact: "The term 'regression' was coined by Sir Francis Galton in the 1800s while studying the heights of parents and children!",
    quiz: { question: "Which algorithm would you use to predict the exact price of a house?", options: ["Clustering", "Classification", "Regression", "Dimensionality Reduction"], answer: "Regression" },
  },
  "Learn:Clustering": {
    title: "Clustering", emoji: "🫧",
    summary: "Clustering is an unsupervised learning technique that groups similar data points together without pre-existing labels.",
    keyPoints: ["Unsupervised Learning: The model finds patterns without knowing the 'right' answers", "K-Means: A popular algorithm that groups data into 'k' distinct clusters", "Used for customer segmentation, anomaly detection, and organizing documents", "The model measures similarity based on the distance between data points"],
    funFact: "Streaming services use clustering to group users with similar viewing habits so they can recommend new shows!",
    quiz: { question: "Clustering is an example of what type of learning?", options: ["Supervised Learning", "Unsupervised Learning", "Reinforcement Learning", "Deep Learning"], answer: "Unsupervised Learning" },
  },
  "Build:Image Classifier": {
    title: "Image Classifier", emoji: "🖼️",
    summary: "An image classifier takes an image as input and predicts what objects or subjects are present within it.",
    keyPoints: ["Typically uses Convolutional Neural Networks (CNNs)", "Requires thousands of labeled images to train effectively", "Used in facial recognition, medical imaging, and self-driving cars", "Data Augmentation (flipping, rotating images) is used to create more training data"],
    funFact: "In 2012, an image classifier called AlexNet revolutionized AI by dominating a major competition using deep learning!",
    quiz: { question: "What type of neural network is most commonly used for image classification?", options: ["Recurrent Neural Networks", "Convolutional Neural Networks", "Transformers", "Generative Adversarial Networks"], answer: "Convolutional Neural Networks" },
  },
  "Build:Chatbot Demo": {
    title: "Chatbot Demo", emoji: "💬",
    summary: "A chatbot uses Natural Language Processing (NLP) to understand human text or speech and generate relevant, conversational responses.",
    keyPoints: ["Rule-based chatbots follow strict if-then logic", "AI chatbots use Large Language Models (LLMs) to understand context", "NLP involves breaking down text into tokens to process meaning", "Used for customer support, virtual assistants, and tutoring"],
    funFact: "ELIZA, created in 1966, was one of the very first chatbots, designed to simulate a psychotherapist!",
    quiz: { question: "What technology allows modern chatbots to understand human language?", options: ["Natural Language Processing (NLP)", "Computer Vision", "Clustering", "Linear Regression"], answer: "Natural Language Processing (NLP)" },
  },
  "Build:Model Playground": {
    title: "Model Playground", emoji: "🎛️",
    summary: "A model playground is an interactive environment where you can tweak settings, change data, and see how a machine learning model reacts in real-time.",
    keyPoints: ["Allows you to experiment with Hyperparameters (like learning rate or network size)", "Helps visualize how models learn to separate or group data", "Great for understanding concepts like Overfitting and Underfitting", "No coding required to see complex math in action"],
    funFact: "Google's TensorFlow Playground lets you build a neural network right in your browser to solve puzzle-like datasets!",
    quiz: { question: "What is the term for the settings you manually tweak in a model (e.g., learning rate)?", options: ["Features", "Hyperparameters", "Labels", "Weights"], answer: "Hyperparameters" },
  },
