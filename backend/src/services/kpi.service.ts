...
export async function computeKpis(
  ...
  const latestLiveYear = latestRow[0]?.latest || null;
  const isLiveYear = !latestLiveYear || academicYear === latestLiveYear || academicYear === currentAcademicYear();
  ...
  if (isLiveYear) {
    ...
    const enrollment = await prisma.$queryRaw<{ class: string; gender: string | null; count: bigint }[]>`
      ...
      WHERE s."schoolId" = ANY(${schoolIds})
        AND (s."studentStatus" = 'Active' OR s."studentStatus" IS NULL)
        AND (s."academicYear" = ${academicYear} OR s."academicYear" IS NULL)
  ...
  const promo = await prisma.$queryRaw<{ result: string; count: bigint }[]>`
    ...
    WHERE b."schoolId" = ANY(${schoolIds})
      AND b."fromAcademicYear" = ${academicYear}
  ...
  result.promotions.pendingBatches = await prisma.promotionBatch.count({
    where: { schoolId: { in: schoolIds }, fromAcademicYear: academicYear, status: 'PENDING_BEO_APPROVAL' },
  });
